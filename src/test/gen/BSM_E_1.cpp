/*
Test pricing of European options under Black-Scholes-Merton, varying the initial
spot price, without importance sampling.
*/
...
{ auto model = fin1::models::BSM
  ...
  constexpr size_t size_S = 100,
                   n_MC   = 1e6,
                   n_S0   = 100;
  const std::string filename = "test_BSM_E_1";
  ...
  const std::shared_ptr<arrow::Buffer>
    ...
    buf_sig = arrow::Buffer::Wrap(data_sig.data(), n_S0),
  ...
  const auto arr_S0  = std::make_shared<arrow::FloatArray>(n_S0, buf_S0 ),
             ...
             arr_sig = std::make_shared<arrow::FloatArray>(n_S0, buf_sig),

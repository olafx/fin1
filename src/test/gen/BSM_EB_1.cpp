/*
Test pricing of European barrier options under Black-Scholes-Merton, varying
various parameters, with importance sampling.
*/
...
int main
()
{ constexpr bool extremely_OTM = true;
  constexpr auto base_model = fin1::models::BSM
  { .S0  = extremely_OTM ? 75 : 100,
    .r   = .05,
    .q   = .02,
    .sig = .2,
    .T   = 1
  ...
  { extremely_OTM ? 65 : 50,
    extremely_OTM ? 80 : 150
                   ...
                   range_K  [] {70, 130},
                   range_B  [] {50, 110};
  // constexpr double expected_ST =
  //   extremely_OTM ? 80 : 140;
  constexpr double rel_err = 0;
  constexpr size_t n_datasets   = 7,
                   size_S       = 100,
                   n_MC_max     = 1e6,
                   size_dataset = 100;
  const std::string filename_base = "test_BSM_EB_1";
  ...
  { auto model  = base_model;
    ...
    { case 0: model.S0  = fin1::math::linspace(range_S0,  i_elem, size_dataset); break;
      ...
      case 3: model.sig = fin1::math::linspace(range_sig, i_elem, size_dataset); break;
      case 4: model.T   = fin1::math::linspace(range_T,   i_elem, size_dataset); break;
      case 5: option.K  = fin1::math::linspace(range_K,   i_elem, size_dataset); break;
      case 6: option.B  = fin1::math::linspace(range_B,   i_elem, size_dataset); break;
    }
  ...
  { const int n_ths = omp_get_max_threads();
    ...
    { std::print("\r{:>{}}/{} {:>{}}/{}",
      ...
      // const double V0 = model.price_1(gen_ths.data(), n_ths, S_ths_C.data(), size_S, option, expected_ST, n_MC_max);
    ...
    const std::vector<std::string> md_k {"model", "option"},
                                   md_v {"Black-Scholes-Merton", "down-and-out European barrier call"};
    ...
    const std::shared_ptr<arrow::Buffer>
      ...
      buf_sig = arrow::Buffer::Wrap(data_sig.data(), size_dataset),
    ...
    const auto arr_S0  = std::make_shared<arrow::FloatArray>(size_dataset, buf_S0 ),
               ...
               arr_sig = std::make_shared<arrow::FloatArray>(size_dataset, buf_sig),
